...
void Regulation::calculatePID(float temperature_actuelle, int current_time) {
    ...
    updateOnOffSchedule(last_pid_output);  // Utilisation de la valeur PID arrondie
}
...
void Regulation::updateRegulation(int current_time, float currentTemp) {
    ...
    if (current_period < onOffSchedule.size() && onOffSchedule[current_period]) {
        regulation_active = currentTemp < consigne;  // Active la régulation si on est dans une période ON et la température est inférieure à la consigne
    } else {
